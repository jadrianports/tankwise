// Shareable trip URLs. Reuses tripState.ts's canonical encode/decode
// UNCHANGED -- this file is not a second serialization format, just the
// reader/writer for window.location.search (there is no router, so this
// reads/writes the query string directly).
...
// falling back to the hero preset for an unknown/absent id rather than
// failing the whole link open.
...
// Decimal strings; the presets themselves are exact floats), find the
// preset that produced it. A hand-tuned WhatIfSliders value that doesn't
// land on any preset falls back to the hero preset id -- the same
// approximation useRecentTrips.ts already accepts for the identical
// reason: the canonical TripState shape stores a preset id, not raw
// slider values.
...
// Builds the absolute shareable URL for the last solved plan. Inputs
// only -- start/finish coordinates and the vehicle profile, never the
// solved plan itself: no secrets, no solved data in the URL.
...
// On first mount, decodes window.location.search (read/write the query
// string directly, no router) via tripState's canonical decodeTripState;
// a valid trip auto-solves immediately with its encoded vehicle profile
// resolved back to a request body, so the loading narration and the
// correct, deterministic result appear on the very first render -- no
// second, self-correcting re-solve.
