...
export interface UseRoutePlanResult {
  ...
  // `waypoints` is optional and additive (WAY-03) -- omitted, a request
  // behaves exactly as before. `vehicle` is an optional one-shot override
  // for the very first submit of a session -- e.g. a share-URL auto-solve,
  // which must send the ENCODED vehicle profile on its one and only
  // request rather than the hero default `vehicleRef` starts with. Every
  // other caller (the planner form, demo chips, recent trips) omits it
  // and gets the currently selected vehicle, unchanged.
  submit: (start: string, finish: string, waypoints?: string[], vehicle?: VehicleProfileRequest) => Promise<void>;
  retry: () => void;
...
export function useRoutePlan(): UseRoutePlanResult {
  ...
  // Last-submitted (start, finish, waypoints) triple -- lets an
  // `upstream_error` state offer a real Retry button that resubmits the
  // SAME request (multi-stop included, never silently collapsing to
  // A->B), without ResultsSection needing to know the last-entered
  // coordinates itself. Also what `resolveVehicle` reuses so a
  // slider/chip change never re-geocodes.
  const lastArgsRef = useRef<{ start: string; finish: string; waypoints?: string[] } | null>(null);
  ...
  const submit = useCallback(
    async (start: string, finish: string, waypoints?: string[], vehicle?: VehicleProfileRequest) => {
      if (vehicle) vehicleRef.current = vehicle;
      lastArgsRef.current = { start, finish, waypoints };
      ...
      let result;
      try {
        result = await planRoute(start, finish, waypoints, vehicleRef.current, controller.signal);
      } catch (err) {
        if (isAbortError(err)) {
  ...
  const retry = useCallback(() => {
    ...
    if (!last) return;
    void submit(last.start, last.finish, last.waypoints);
  }, [submit]);
