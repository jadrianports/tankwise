...
| `MAPBOX_DAILY_CALL_CAP` / `MAPBOX_MONTHLY_CALL_CAP` | `3000` / `60000` | Deployment-wide ceilings on outbound Mapbox calls (Directions and Geocoding counted separately). The two throttle rates above are per-client-IP, so they bound one caller but not the total; these bound the total. Both sit well under Mapbox's 100,000/month free tiers. Set either to `0` to disable that window. |
| `MAPBOX_BUDGET_ENABLED` | `true` | Kill switch for the call budget above. |
...
| `EIA_API_KEY` | *(none, optional)* | Enables live weekly indexing of station prices to `api.eia.gov` regional diesel averages. Register a free key instantly at [eia.gov/opendata](https://www.eia.gov/opendata/). Unset = the app runs in frozen-snapshot mode (the original 2024/2025 prices, unindexed), which is the default for local dev/CI/reviewers. |
| `FUEL_PRICE_AS_OF` | `2025-01-01` | Display-only metadata qualifying every price/cost figure the API returns when running in frozen-snapshot mode. See "Free-tier deployment" below for how this date was derived. |
...
1. **Cold starts.** Render spins the free-tier instance down after about 15 minutes with no inbound traffic. The first request after that idle window wakes it back up, which can take roughly a minute; every request after that is fast again. An external cron pings `GET /api/health` every ~10 minutes to keep the instance warm during normal use, so this mostly only shows up after a long gap between visits, not mid-demo. There are two different wake paths, and this app only owns one of them. If the page is already open when the instance falls asleep, the app notices the wait itself and puts up its own "waking up the server" overlay -- because Render holds the connection open rather than failing it, this is detected from elapsed time, set deliberately above the slowest measured warm solve so an expensive corridor is never mislabelled as a sleeping server. If you arrive cold, the instance is still asleep when the browser asks for the page itself, so Render's own wake screen answers first and this app's JavaScript never gets a chance to run -- that screen is the intended experience there, and owning that case too would mean serving the front end from a separate always-on host, a trade this single-image deployment deliberately does not make.
...
| 503 | `upstream_budget_exhausted` | The deployment-wide outbound-call budget for the current day or month is spent (`MAPBOX_DAILY_CALL_CAP` / `MAPBOX_MONTHLY_CALL_CAP`). Distinct from a 429, which means *this* caller was too chatty; this means the service as a whole is out of allowance. Already-cached routes keep serving normally, since the cache is read before any upstream call. | `{"error":{"code":"upstream_budget_exhausted","message":"Routing is temporarily unavailable: this deployment's upstream request budget for the current window is used up.","detail":{"window":"day","cap":3000}}}` |
...
- **Backend:** `python manage.py test` runs the full Django/DRF suite. It covers the solver's edge cases (the exact 500-mile boundary, an infeasible gap, a single candidate station, a sub-500-mile trip that needs no stops, and the trap case where a farther but cheaper station still has to win over a nearer, pricier one — a property that predates the Phase 18 DP swap and both solver arms must still honor), the corridor filter's geometry, the Mapbox client and its error mapping, the cache-key normalizer, the full serializer and response contract, and the `/api/route` and `/api/health` views end to end.
- **Frontend:** `npm test` (run from `frontend/`) runs the Vitest suite — 150 tests across 23 files, spanning the pure-logic modules (the per-error-code message mapper, formatting and quantile helpers), the export and trip-state layers (CSV/GeoJSON export, share-URL round-tripping, recent-trips storage), the vehicle what-if hook and its debounced resolve behavior, and the presentational component suite (summary card, stop list, playback HUD, results panel and their error/loading branches). Coverage is measured across the whole `src` surface, not just the files a test happens to import, and currently sits at just under 57% of statements; the imperative Mapbox GL integration modules (`MapView.tsx`, `useChaseCam.ts`) are excluded from that figure for the documented reason in `frontend/vite.config.ts` — a jsdom unit test against them would mostly assert the mocked SDK behaves, not that the app does. `npm run test:coverage` writes an lcov report to `frontend/coverage/`.
