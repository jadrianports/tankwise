"""Frozen copy of `routing/services/solver.py` exactly as it existed before
Phase 18. Constructed via:

    git show 6849e75a075b0e1cc8bff7c12e5f21830641d75a:routing/services/solver.py

Test-only: never imported by production code (`routing/views.py`,
`routing/services/`). This is D-07's behavioural referee -- the
differential in `routing/tests/test_dp_differential.py` asserts the DP at
`penalty=0` matches this module's `solve()` on stops, gallons, cost and
every `purchase_reason`.
